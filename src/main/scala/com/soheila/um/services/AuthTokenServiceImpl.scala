...
import com.soheila.um.daos.AuthTokenDAO
...
class AuthTokenServiceImpl @Inject() (authTokenDAO: AuthTokenDAO, clock: Clock) extends AuthTokenService {
  ...
  def create(userID: String, expiry: FiniteDuration = 5 minutes) = {
    val token = AuthToken(UUID.randomUUID(), UUID.fromString(userID), clock.now.withZone(DateTimeZone.UTC).plusSeconds(expiry.toSeconds.toInt))
