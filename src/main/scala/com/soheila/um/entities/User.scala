...
object User {
  ...
  implicit object StoryIdentity extends IdentityWithAudit[User, String] {
    ...
    override def addAuditTrail(entity: User): User = entity.copy(createdOn = Some(LocalDateTime.now(Clock.systemUTC())), updatedOn = Some(LocalDateTime.now(Clock.systemUTC())))

    override def updateAuditTrail(entity: User): User = entity.copy(updatedOn = Some(LocalDateTime.now(Clock.systemUTC())))
  }
