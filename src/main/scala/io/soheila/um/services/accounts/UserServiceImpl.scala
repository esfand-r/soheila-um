 ...
 */
class UserServiceImpl @Inject() (userDAO: UserDAO, @Named("activity-manager") activityManager: ActorRef) extends UserService {
  ...
  def save(user: User) = userDAO.create(user).map {
    case Left(dAOException) => throw UserServiceException(dAOException.message, dAOException)
    case Right(s) => s
  }
  ...
  def save(profile: CommonSocialProfile) = {
    userDAO.find(profile.loginInfo).flatMap {
      ...
      case None => // Insert a new user
        ...
        userDAO.create(userToCreate).map {
          case Left(dAOException) => throw UserServiceException(dAOException.message, dAOException)
          case Right(s) => s
        }
    }
