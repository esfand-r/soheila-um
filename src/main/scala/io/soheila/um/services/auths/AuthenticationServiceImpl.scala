...
class AuthenticationServiceImpl @Inject() (
  ...
  override def signUp(signUpData: SignUpVO)(implicit request: RequestHeader): Future[String] = {
    ...
    userService.retrieve(loginInfo).flatMap {
      case Some(user) => Future.failed(AuthenticationException("user.exists", null))
      case None =>
      ...
      case e @ (_: AuthenticationException) =>
        logger.error(e.getMessage, e)
        throw e
      case e => throw AuthenticationException("invalid.credentials", e)
    }
  }
  ...
  override def resetPassword(authToken: AuthToken, resetPasswordData: ResetPasswordVO)(implicit request: RequestHeader): Future[Boolean] = {
    userService.retrieve(authToken.userID.toString).flatMap {
      case Some(user) if user.loginInfo.providerID == CredentialsProvider.ID =>
        ...
        authInfoRepository.update[PasswordInfo](user.loginInfo, passwordInfo).map {
          _ => true
        }
      case _ => Future.failed(AuthenticationException("invalid.reset.link", null))
    }.recover {
      case e =>
        logger.error(e.getMessage, e)
        throw AuthenticationException("invalid.reset.link", e)
    }
  }
  ...
  override def changePassword(credentials: Credentials, changePasswordData: ChangePasswordVO)(implicit request: RequestHeader): Future[Boolean] = {
    credentialsProvider.authenticate(credentials).flatMap { loginInfo =>
      ...
      case e =>
        logger.error(e.getMessage, e)
        throw AuthenticationException("current.password.invalid", e)
    }
  }
  ...
  override def signIn(signInData: SignInVO)(implicit request: RequestHeader): Future[Token] = {
    credentialsProvider.authenticate(Credentials(signInData.email, signInData.password)).flatMap { loginInfo =>
      ...
      val epochMilli = clock.now.atZone(ZoneId.systemDefault()).toInstant.toEpochMilli
      val jodaLocalDateTime: org.joda.time.DateTime = new org.joda.time.DateTime(epochMilli)
      val expirationDateTime = jodaLocalDateTime + c.as[FiniteDuration]("silhouette.authenticator.rememberMe.authenticatorExpiry")
  ...
  override def socialSignIn[B](provider: String)(implicit request: ExtractableRequest[B]): Future[Either[Result, String]] = {
    (socialProviderRegistry.get[SocialProvider](provider) match {
      ...
      case e =>
        logger.error(e.getMessage, e)
        throw AuthenticationException("could.not.authenticate", e)
    }
  }
}
